/**
 * Specs Inc. 2026
 * MLController for the SnapML Pool Spectacles lens experience.
 */
...
export class MLController extends BaseScriptComponent {
  @ui.label('<span style="color: #60A5FA;">MLController – YoloV7 ML inference and detection pipeline</span><br/><span style="color: #94A3B8; font-size: 11px;">Runs the ML model on cropped camera frames and forwards detections to PoolTablePredictor.</span>')
  @ui.separator

  @ui.label('<span style="color: #60A5FA;">References</span>')
  @input()
  @hint("Camera service providing the cropped input texture and pose data")
  cameraService: CameraService

  @input()
  @hint("Pool table predictor that receives and processes ML detections")
  poolTablePredictor: PoolTablePredictor

  @input()
  @hint("ONNX ML asset for the YoloV7-tiny 512x512 pool detection model")
  model: MLAsset

  @input()
  @hint("Detection controller used to visualize bounding boxes in the debug overlay")
  detectionController: DetectionController

  @ui.separator
  @ui.label('<span style="color: #60A5FA;">Detection Settings</span>')
  @input()
  @hint("Minimum confidence score to consider a detection valid")
  @widget(new SliderWidget(0, 1, 0.01))
  scoreThreshold: number = 0.33

  @input()
  @hint("Intersection-over-union threshold for non-maximum suppression")
  @widget(new SliderWidget(0, 1, 0.01))
  ...
  onAwake() {
    this.logger = new Logger("MLController", this.enableLogging || this.enableLoggingLifecycle, true)
  ...
  private onUpdate(runTimeStamp: number) {
    ...
    if (this.currentFrame >= frameSkip && !this.isRunning) {
      ...
      if (this.cameraService.isEditor) {
        ...
        this.mlComponent.runImmediate(runImmediate)
        this.logger.debug("runImmediate: " + runImmediate)
      }
    }
  }
