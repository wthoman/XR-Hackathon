...
import {EditableUserPosition} from "SpectaclesNavigationKit.lspkg/NavigationDataComponent/EditableUserPosition"
...
export class MapComponent extends BaseScriptComponent {
  ...
  onStart() {
    ...
    if (this.setMapToCustomLocation && mapLocation) {
      const userPos = this.userPosition as EditableUserPosition
      userPos.setOverridePosition(mapLocation)
    }
  }
  ...
  createMapPin(longitude: number, latitude: number, visitDistance: number): MapPin {
    const location = {longitude, latitude} as unknown as GeoPosition
