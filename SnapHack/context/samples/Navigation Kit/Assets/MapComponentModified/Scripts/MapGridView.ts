...
export class MapGridView {
  ...
  static makeGridView(mapController: MapController, mapScaleX: number, mapScaleY: number): MapGridView {
    const gridView = new MapGridView()
    gridView.mapScaleX = mapScaleX
    gridView.mapScaleY = mapScaleY
  ...
  updateGridView(mapPins: Set<MapPin>, userPin: MapPin) {
    ...
    const topLeftCorner = this.config.gridScreenTransform.localPointToWorldPoint(
      new vec2(-this.mapScaleX, this.mapScaleY)
    )
