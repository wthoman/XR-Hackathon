...
import {bindDestroyEvent} from "SnapDecorators.lspkg/decorators"
...
export class InteractableManipulation extends BaseScriptComponent {
  ...
  onAwake(): void {
    this.logger = new Logger("InteractableManipulation", this.enableLogging || this.enableLoggingLifecycle, true)
  ...
  private setupColliders(isMiniMap: boolean): void {
    ...
    if (shape.isOfType("BoxShape")) {
      ...
      this.logger.error("Other shapes of collider is not currently supported for map interaction")
    }
  }
  ...
  private updateStartValues(): void {
    ...
    if (interactor !== null) {
      if (this.isInteractorValid(interactor) === false) {
        this.logger.error("Interactor must not be valid for setting initial values")
        return
      }
  ...
  private onHoverToggle(eventData: InteractorEvent): void {
    ...
    if (this.hoveringInteractor !== null) {
      this.logger.info("On hover Start Event")
    } else {
      this.logger.info(`On hover End Event`)
    }
  }
  ...
  private onTriggerToggle(eventData: InteractorEvent): void {
    ...
    if (this.triggeringInteractor !== null) {
      ...
      this.logger.info("InteractionEvent : " + "On Manipulation Start Event")
    } else {
      ...
      this.logger.info("InteractionEvent : " + "On Manipulation End Event")
    }
  }
  ...
  private singleInteractorTransform(interactor: Interactor): void {
    if (this.isInteractorValid(interactor) === false) {
      this.logger.error("Interactor must be valid")
      return
    }
    validate(this.mapTransform)
