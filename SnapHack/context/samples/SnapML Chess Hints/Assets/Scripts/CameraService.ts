/**
 * Specs Inc. 2026
 * Defines Camera Service, Camera Pose With Timestamp for the SnapML Chess Hints lens.
 */
...
export class CameraService extends BaseScriptComponent {
  @ui.label('<span style="color: #60A5FA;">CameraService – device camera access and pose tracking</span><br/><span style="color: #94A3B8; font-size: 11px;">Manages camera setup, crop provider, and timestamped pose history.</span>')
  @ui.separator

  @ui.label('<span style="color: #60A5FA;">Camera References</span>')
  @input
  @hint("Main scene camera used for world transform tracking")
  mainCamera: Camera

  @input
  @hint("Editor-only camera used to simulate device camera in Lens Studio")
  editorCamera: Camera

  @ui.separator
  @ui.label('<span style="color: #60A5FA;">ML & Textures</span>')
  @input
  @hint("ML controller to initialize after camera crop is ready")
  MLController: MLController

  @input
  @hint("Crop texture fed into the ML model as input")
  screenCropTexture: Texture

  @input
  @hint("Render texture used for the editor camera feed")
  editorRenderTexture: Texture

  @ui.separator
  @ui.label('<span style="color: #60A5FA;">Debug</span>')
  @input
  @hint("Debug image overlay for visualizing camera output")
  debugImage: Image

  @input
  @hint("Render target used for label overlay rendering")
