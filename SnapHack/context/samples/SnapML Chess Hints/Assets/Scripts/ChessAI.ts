/**
 * Specs Inc. 2026
 * Defines Move, Chess AI, Fen Result for the SnapML Chess Hints lens.
 */
...
export class Move {
  ...
  constructor(data: any) {
    ...
    if (data["from"] != undefined) {
      ...
      throw new Error("Invalid move notation: " + data)
    }
  }
...
export class ChessAI {
  ...
  constructor(useStockfish: boolean, logger?: Logger) {
  ...
  updateFEN(FEN: string) {
    ...
    if (!validation.ok) {
      this.logger?.warn("Invalid FEN: " + validation.error + " " + FEN)
      return false
    } else if (validation.correctedFen) {
      //this.logger?.debug("Corrected FEN: " + validation.correctedFen)
    }
  ...
  async fetchMove(
    ...
    this.logger?.debug("fetching move: " + FEN)
    ...
    Gemini.models(request)
      .then((response) => {
        ...
        if (bodyJson.from == undefined || bodyJson.to == undefined || bodyJson.piece == undefined) {
          ...
          this.logger?.debug(this.suggestedMove.from + " -> " + this.suggestedMove.to)
          onResponse({
            success: true,
            message: "",
            fen: FEN,
