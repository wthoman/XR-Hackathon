/**
 * Specs Inc. 2026
 * Diagram Storage component for the Agentic Playground Spectacles lens.
 */
...
export class DiagramStorage extends BaseScriptComponent {
  @ui.label('<span style="color: #60A5FA;">DiagramStorage</span>')
  @ui.separator
  ...
  onAwake(): void {
    this.logger = new Logger("DiagramStorage", this.enableLogging || this.enableLoggingLifecycle, true)
  ...
  public createNewDiagram(title: string): void {
    ...
    this.generateDiagramDocument()
          ...
          this.logger.info(`Created new diagram "${title}"`)
  }
  ...
  public addNode(nodeType: "text" | "image" | "model", title: string, content: string, prompt?: string): void {
    if (!this.currentDiagramTitle) {
      this.logger.info("No active diagram - creating default diagram")
    ...
    this.generateDiagramDocument()
          ...
          this.logger.info(`Added ${nodeType} node "${title}" to diagram "${this.currentDiagramTitle}"`)
  }
  ...
  public saveDiagram(): void {
    if (!this.currentDiagramTitle || this.currentNodeCount === 0) {
      this.logger.info("No diagram to save")
      return
    }
    ...
    this.cleanupOldDiagrams()
          ...
          this.logger.info(`Saved diagram "${this.currentDiagramTitle}" with ID ${diagramId}`)
  }
  ...
  public loadDiagram(diagramId: string): DiagramDocument | null {
    ...
    if (document) {
      ...
      this.logger.info(`Loaded diagram "${document.diagramTitle}"`)
    } else {
      this.logger.info(`Diagram with ID ${diagramId} not found`)
    }
  ...
  public updateDiagram(nodes: DiagramNode[]): void {
    ...
    this.generateDiagramDocumentFromNodes(nodes)
          ...
          this.logger.info(`Updated diagram "${this.currentDiagramTitle}" with ${nodes.length} nodes`)
  }
  ...
  public clearAllDiagrams(): void {
    ...
    this.saveToStorage()
          ...
          this.logger.info("Cleared all stored diagrams")
  }
  ...
  private initializeStorage(): void {
    ...
    try {
      ...
      this.logger.info(`Storage system initialized with ${this.storedDiagrams.size} stored diagrams`)
    } catch (error) {
  ...
  private cleanupOldDiagrams(): void {
    ...
    for (let i = 0; i < excessCount; i++) {
      ...
      this.logger.info(`Removed old diagram ${oldestId} to maintain storage limit`)
    }
  }
    ...
    try {
      if (global.persistentStorageSystem) {
        ...
        if (storedDataString && storedDataString.length > 0) {
          ...
          this.logger.info(`📖 Loaded ${this.storedDiagrams.size} diagrams from storage`)
        }
      }
    } catch (error) {
