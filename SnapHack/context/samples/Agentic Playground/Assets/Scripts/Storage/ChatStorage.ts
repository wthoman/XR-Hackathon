/**
 * Specs Inc. 2026
 * Chat Storage component for the Agentic Playground Spectacles lens.
 */
...
export class ChatStorage extends BaseScriptComponent {
  @ui.label('<span style="color: #60A5FA;">ChatStorage</span>')
  @ui.separator
  ...
  onAwake(): void {
    this.logger = new Logger("ChatStorage", this.enableLogging || this.enableLoggingLifecycle, true)
  ...
  public startNewSession(title?: string): ChatSession {
    ...
    this.onSessionStarted.invoke(this.currentChatSession)
          ...
          this.logger.info(`Started new session: "${sessionTitle}" (${sessionId})`)
  ...
  public endCurrentSession(): ChatSession | null {
    if (!this.currentChatSession) {
      this.logger.info("No active session to end")
      return null
    }
    ...
    this.onSessionEnded.invoke(session)
          ...
          this.logger.info(`Session ended: "${session.sessionTitle}" with ${session.totalMessages} messages`)
  ...
  public addMessage(message: ChatMessage): void {
    ...
    this.onMessageAdded.invoke(message)
          ...
          this.logger.info(`Message added: ${message.type} - "${message.content.substring(0, 50)}..."`)

    // Check if session needs archiving due to size
    if (this.currentChatSession.totalMessages >= this.maxMessagesPerSession) {
      this.logger.info(`📦 Session reached max messages (${this.maxMessagesPerSession}), archiving...`)
      this.archiveCurrentSession()
    }
  ...
  public clearAllMemory(): void {
    ...
    this.saveToStorage()
          ...
          this.logger.info("All chat memory cleared (user requested memory deletion)")
  }
  ...
  public clearSession(sessionId: string): boolean {
    if (this.storedSessions.has(sessionId)) {
      ...
      this.logger.info(`Session ${sessionId} cleared`)
      return true
    }
    return false
  }
  ...
  private archiveCurrentSession(): ChatSession | null {
    ...
    this.saveToStorage()
          ...
          this.logger.info(`📦 Session archived: "${archived.sessionTitle}"`)
  ...
  private cleanupOldSessions(): void {
    ...
    toRemove.forEach(([sessionId, _]) => {
      this.storedSessions.delete(sessionId)
          ...
          this.logger.info(`Cleaned up ${toRemove.length} old sessions`)
  }
  ...
  private initializeStorage(): void {
    try {
      ...
      this.logger.info(`Initialized with ${this.storedSessions.size} stored sessions`)
      if (this.currentChatSession) {
        this.logger.info(`📋 Active session: "${this.currentChatSession.sessionTitle}"`)
      }
    } catch (error) {
    ...
    try {
      if (global.persistentStorageSystem) {
        ...
        if (storedDataString && storedDataString.length > 0) {
          ...
          this.logger.info(`📖 Loaded ${this.storedSessions.size} sessions from storage`)
        }
      }
    } catch (error) {
