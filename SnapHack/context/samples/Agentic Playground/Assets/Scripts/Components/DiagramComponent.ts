/**
 * Specs Inc. 2026
 * Defines YSeparation Animation, Node Mapping Reference, Starting Node Mapping Reference for the
 * Agentic Playground lens.
 */
...
export class DiagramComponent extends BaseScriptComponent {
  @ui.label('<span style="color: #60A5FA;">DiagramComponent</span>')
  @ui.separator
  ...
  onAwake(): void {
    this.logger = new Logger("DiagramComponent", this.enableLogging || this.enableLoggingLifecycle, true)
  ...
  private performCollisionAvoidance(): void {
    ...
    this.logger.info("SimpleMindNodeManager: Performing collision avoidance")

    const conflicts = TreeStructureUtils.findNodesNeedingYSeparation(this.nodeReferences, this.minXDistance)

    if (conflicts.length > 0) {
      this.logger.info(`SimpleMindNodeManager: Found ${conflicts.length} X-distance conflicts, applying Y separation`)
      ...
      this.logger.info("SimpleMindNodeManager: No X-distance conflicts found")
    }
  }
  ...
  private startYSeparationAnimation(node: NodeMappingReference, targetPosition: vec3): void {
    const animation = new YSeparationAnimation(
      ...
      () => {
        this.logger.info(`SimpleMindNodeManager: Y separation animation completed for ${node.nodeId}`)
      }
    )

    this.activeYAnimations.set(node.nodeId, animation)
    this.logger.info(`SimpleMindNodeManager: Started Y separation animation for ${node.nodeId}`)
  }

  // Test Phase 0: Generate starting node with random name
  private testPhase0(): void {
    this.logger.info("SimpleMindNodeManager: Starting Phase 0 - Creating starting node")
    this.clearAllNodes()
    ...
    this.logger.info(`SimpleMindNodeManager: Created starting node with diagram name: ${randomDiagramName}`)
  }

  // Test Phase 1: Create 3 main branches from starting node (Level 1)
  private testPhase1(): void {
    this.logger.info("SimpleMindNodeManager: Starting Phase 1 - Creating Level 1 tree structure")
    ...
    if (!this.startingNode) {
      this.logger.info("SimpleMindNodeManager: Error - No starting node available")
      return
    }
    ...
    this.logger.info("SimpleMindNodeManager: Phase 1 completed - Level 1 tree structure created")
  }

  // Test Phase 2: Create Level 2 nodes from Level 1 nodes
  private testPhase2(): void {
    this.logger.info("SimpleMindNodeManager: Starting Phase 2 - Creating Level 2 tree structure")
    ...
    this.logger.info("SimpleMindNodeManager: Phase 2 completed - Level 2 tree structure created")
  }

  // Test Phase 3: Modify some Level 2 nodes (variations)
  private testPhase3(): void {
    this.logger.info("SimpleMindNodeManager: Starting Phase 3 - Creating variations")

    // Ensure we have the prerequisite structure
    if (this.nodeReferences.size <= 4) {
      this.logger.info("SimpleMindNodeManager: Phase 3 requires previous phases - executing Phase 2 first")
      this.testPhase2()
    }
    ...
    if (level2Nodes.length === 0) {
      this.logger.info("SimpleMindNodeManager: No Level 2 nodes found - executing Phase 2 first")
      this.testPhase2()
    }
    ...
    this.logger.info(`SimpleMindNodeManager: Modifying ${nodesToModify} out of ${currentLevel2Nodes.length} Level 2 nodes`)
    ...
    for (let i = 0; i < nodesToModify && i < currentLevel2Nodes.length; i++) {
      ...
      if (parent) {
        this.logger.info(`SimpleMindNodeManager: Replacing node ${nodeToRemove.nodeId} with variation`)
    ...
    this.logger.info("SimpleMindNodeManager: Phase 3 completed - Variations created")
  }

  // Test Phase 4: Continue growing the tree
  private testPhase4(): void {
    this.logger.info("SimpleMindNodeManager: Starting Phase 4 - Continue growing")

    // Ensure we have the prerequisite structure (similar to phase 2)
    if (this.nodeReferences.size <= 4) {
      this.logger.info("SimpleMindNodeManager: Phase 4 requires previous phases - executing Phase 2 first")
      this.testPhase2()
    }
    ...
    this.logger.info(`SimpleMindNodeManager: Found ${currentLevel2Nodes.length} Level 2 nodes to grow from`)
    ...
    for (const parentNode of currentLevel2Nodes) {
      if (Math.random() < 0.5) {
        ...
        if (branchCount > 0) {
          this.logger.info(`SimpleMindNodeManager: Adding ${branchCount} branches to Level 2 node ${parentNode.nodeId}`)
    ...
    this.logger.info("SimpleMindNodeManager: Phase 4 completed - Tree growth continued")
  }
    ...
    if (imageNodeRef) {
      ...
      if (imageNodeRef.generateContent) {
        imageNodeRef.generateContent(imagePrompt).catch((error) => {
          this.logger.info(`DiagramComponent: Image generation failed for ${nodeId}: ${error}`)
        })

        this.logger.info(`DiagramComponent: Triggered image generation for ${nodeId} with prompt: "${imagePrompt}"`)
      } else {
        this.logger.info(`DiagramComponent: generateContent method not available on ImageNode ${nodeId}`)
      }
    }
    ...
    if (modelNodeRef) {
      ...
      if (modelNodeRef.generateContent) {
        modelNodeRef.generateContent(modelPrompt).catch((error) => {
          this.logger.info(`DiagramComponent: Model generation failed for ${nodeId}: ${error}`)
        })

        this.logger.info(`DiagramComponent: Triggered model generation for ${nodeId} with prompt: "${modelPrompt}"`)
      } else {
        this.logger.info(`DiagramComponent: generateContent method not available on ModelNode ${nodeId}`)
      }
    }
  ...
  private setupManipulationResetBehavior(
    ...
    try {
      ...
      this.logger.info(`SimpleMindNodeManager: Could not access InteractableManipulation on ${nodeObject.name} - Error: ${error}`)
      return
    }
    ...
    if (manipulationComponent && manipulationComponent.onManipulationStart && manipulationComponent.onManipulationEnd) {
      ...
      this.logger.info(`SimpleMindNodeManager: Setup manipulation reset for ${nodeObject.name} (${nodeType})`)
    } else {
      this.logger.info(`SimpleMindNodeManager: InteractableManipulation component or events missing on ${nodeObject.name}`)
      if (manipulationComponent) {
        this.logger.info(`SimpleMindNodeManager: Available properties: ${Object.keys(manipulationComponent).join(", ")}`)
      }
    }
  }
  ...
  private startManipulation(nodeObject: SceneObject, logicalPosition: vec3, nodeType: string): void {
    ...
    if (!nodeId) {
      this.logger.info(`SimpleMindNodeManager: Could not find node ID for manipulation start`)
      return
    }
    ...
    this.logger.info(`SimpleMindNodeManager: Started manipulation for ${nodeObject.name} (${nodeType})`)
  }

  private endManipulation(nodeObject: SceneObject): void {
    const nodeId = this.getNodeIdFromObject(nodeObject)
    if (!nodeId) {
      this.logger.info(`SimpleMindNodeManager: Could not find node ID for manipulation end`)
      return
    }

    const manipulationState = this.manipulationStates.get(nodeId)
    if (!manipulationState || !manipulationState.isManipulating) {
      this.logger.info(`SimpleMindNodeManager: No active manipulation state found for ${nodeObject.name}`)
      this.logger.info(`SimpleMindNodeManager: Total manipulation states: ${this.manipulationStates.size}`)
      return
    }
    ...
    this.logger.info(`SimpleMindNodeManager: Reset position for ${nodeObject.name}`)
  }
  ...
  public addTextNode(nodeId: string, content: string, position?: vec3, level: number = 1): boolean {
    try {
      ...
      if (newNode) {
        this.logger.info(`AgenticDiagram: Added text node "${nodeId}"`)
        return true
      }

      return false
    } catch (error) {
      this.logger.info(`AgenticDiagram: Error adding text node: ${error}`)
      return false
    }
  }
  ...
  public addImageNode(nodeId: string, content: string, position?: vec3, level: number = 1): boolean {
    try {
      ...
      if (newNode) {
        this.logger.info(`AgenticDiagram: Added image node "${nodeId}"`)
        return true
      }

      return false
    } catch (error) {
      this.logger.info(`AgenticDiagram: Error adding image node: ${error}`)
      return false
    }
  }
  ...
  public addModelNode(nodeId: string, content: string, position?: vec3, level: number = 1): boolean {
    try {
      ...
      if (newNode) {
        this.logger.info(`AgenticDiagram: Added model node "${nodeId}"`)
        return true
      }

      return false
    } catch (error) {
      this.logger.info(`AgenticDiagram: Error adding model node: ${error}`)
      return false
    }
  }
  ...
  public addConnection(fromNodeId: string, toNodeId: string): boolean {
    try {
      ...
      if (!fromNode || !toNode) {
        this.logger.info(`AgenticDiagram: Cannot create connection - node not found: ${fromNodeId} -> ${toNodeId}`)
        return false
      }
      ...
      if (lineRef) {
        this.logger.info(`AgenticDiagram: Added connection: ${fromNodeId} -> ${toNodeId}`)
        return true
      }

      return false
    } catch (error) {
      this.logger.info(`AgenticDiagram: Error adding connection: ${error}`)
      return false
    }
  }
  ...
  public printTreeStructure(): void {
    ...
    this.logger.info(`SimpleMindNodeManager Tree Structure:`)
    this.logger.info(`- Total Nodes: ${stats.totalNodes}`)
    this.logger.info(`- Max Level: ${stats.maxLevel}`)
    this.logger.info(`- Current Phase: ${this.currentPhase}`)

    for (const [level, count] of stats.nodesByLevel) {
      this.logger.info(`  Level ${level}: ${count} nodes`)
    }
  }

  public printSystemStatus(): void {
    this.logger.info(`SimpleMindNodeManager Status:`)
    this.logger.info(`- Current Phase: ${this.currentPhase}`)
    this.logger.info(`- Nodes Count: ${this.nodeReferences.size}`)
    this.logger.info(`- Lines Count: ${this.lineReferences.size}`)
    this.logger.info(`- Level Separation: ${this.levelSeparation}`)
    this.logger.info(`- Angular Spread: ${this.angularSpread}`)
    this.logger.info(`- Min Branch Distance: ${this.minBranchDistance}`)
    this.logger.info(`- Max Branches: ${this.maxBranches}`)
    this.logger.info(`- Min X Distance: ${this.minXDistance}`)
    this.logger.info(`- Max Y Offset: ${this.maxYOffset}`)
    this.logger.info(`- Active Y Animations: ${this.activeYAnimations.size}`)
    this.logger.info(`- Y Separation Enabled: ${this.enableYSeparation}`)
    this.logger.info(`- Y Variation Enabled: ${this.enableYVariation}`)
    this.logger.info(`- Max Y Variation: ${this.maxYVariation}`)
    this.logger.info(`- Node Y Offset: ${this.nodeYOffset}`)
    this.logger.info(`- Image Y Offset Multiplier: ${this.imageYOffsetMultiplier}`)
  }

  public forceCollisionAvoidance(): void {
    this.logger.info("SimpleMindNodeManager: Manually triggering collision avoidance")
    this.performCollisionAvoidance()
  }

  public printCollisionStatus(): void {
    this.logger.info("SimpleMindNodeManager Collision Status:")

    const conflicts = TreeStructureUtils.findNodesNeedingYSeparation(this.nodeReferences, this.minXDistance)

    this.logger.info(`- Current X-distance conflicts: ${conflicts.length}`)
    this.logger.info(`- Active Y animations: ${this.activeYAnimations.size}`)
    this.logger.info(`- Min X distance threshold: ${this.minXDistance}`)
    this.logger.info(`- Max Y offset: ${this.maxYOffset}`)
  ...
  public addRandomYVariation(): void {
    this.logger.info("SimpleMindNodeManager: Adding random Y variation to all nodes")
  ...
  public executeSequentialPhases(targetPhase: number): void {
    this.logger.info(`SimpleMindNodeManager: Executing phases sequentially up to phase ${targetPhase}`)

    for (let phase = 0; phase <= targetPhase; phase++) {
      this.logger.info(`SimpleMindNodeManager: --> Executing phase ${phase}`)
    ...
    this.logger.info(`SimpleMindNodeManager: Sequential execution completed up to phase ${targetPhase}`)
  }

  public printPhaseStructure(): void {
    this.logger.info("SimpleMindNodeManager Phase Structure:")
    this.logger.info(`Current Phase: ${this.currentPhase}`)
    this.logger.info(`Total Nodes: ${this.nodeReferences.size}`)
    this.logger.info(`Total Lines: ${this.lineReferences.size}`)
    ...
    this.logger.info(`Node Types: Start(${startNodes}) Text(${textNodes}) Image(${imageNodes}) Model(${modelNodes})`)
  }

  public updateAllNodeVisualPositions(): void {
    this.logger.info(`SimpleMindNodeManager: Updating all node visual positions with Y offset: ${this.nodeYOffset}`)
    ...
    this.logger.info(`SimpleMindNodeManager: Updated ${this.nodeReferences.size} node positions`)
  }

  public printPositionInfo(): void {
    this.logger.info("SimpleMindNodeManager Position Info:")
    this.logger.info(`Node Y Offset: ${this.nodeYOffset}`)
    this.logger.info(`Image Y Offset Multiplier: ${this.imageYOffsetMultiplier}`)
    ...
    for (const node of this.nodeReferences.values()) {
      ...
      this.logger.info(`Node ${node.nodeId} (${nodeType}):`)
      this.logger.info(`  Logical: (${logicalPos.x.toFixed(1)}, ${logicalPos.y.toFixed(1)}, ${logicalPos.z.toFixed(1)})`)
      this.logger.info(`  Visual: (${visualPos.x.toFixed(1)}, ${visualPos.y.toFixed(1)}, ${visualPos.z.toFixed(1)})`)
      this.logger.info(
  ...
  public printManipulationStatus(): void {
    this.logger.info("SimpleMindNodeManager Manipulation Status:")
    this.logger.info(`- Active manipulation states: ${this.manipulationStates.size}`)

    for (const [nodeId, state] of this.manipulationStates) {
      this.logger.info(`  Node ${nodeId} (${state.nodeType}): ${state.isManipulating ? "MANIPULATING" : "IDLE"}`)
      if (state.isManipulating) {
        ...
        this.logger.info(`    Current: (${currentPos.x.toFixed(1)}, ${currentPos.y.toFixed(1)}, ${currentPos.z.toFixed(1)})`)
      }
    }
  }

  public testManipulationComponents(): void {
    this.logger.info("SimpleMindNodeManager: Testing manipulation components on all nodes...")
    ...
    for (const [nodeId, node] of this.nodeReferences) {
      ...
      try {
        ...
        if (manipComponent) {
          this.logger.info(`InteractableManipulation component found`)
          this.logger.info(`  Properties: ${Object.keys(manipComponent).join(", ")}`)

          // Test event availability
          if (manipComponent.onManipulationStart) {
            this.logger.info(`  onManipulationStart event available`)
          } else {
            this.logger.info(`  onManipulationStart event NOT available`)
          }

          if (manipComponent.onManipulationEnd) {
            this.logger.info(`  onManipulationEnd event available`)
          } else {
            this.logger.info(`  onManipulationEnd event NOT available`)
          }
        } else {
          this.logger.info(`InteractableManipulation component is null`)
        }
      } catch (error) {
        this.logger.info(`Error accessing InteractableManipulation: ${error}`)
      }
    }
  }

  public debugManipulationSetup(): void {
    this.logger.info("SimpleMindNodeManager: Debug manipulation setup...")
    this.logger.info(`Total nodes: ${this.nodeReferences.size}`)
    this.logger.info(`Total manipulation states: ${this.manipulationStates.size}`)
    ...
    this.logger.info("SimpleMindNodeManager: Debug manipulation setup completed!")
  }

  public testSingleNodeManipulation(nodeId?: string): void {
    this.logger.info("SimpleMindNodeManager: Testing single node manipulation...")
    ...
    if (!node) {
      this.logger.info(`SimpleMindNodeManager: Node ${targetNodeId} not found`)
      return
    }

    this.logger.info(`SimpleMindNodeManager: Testing manipulation reset for ${targetNodeId} (${node.nodeName})`)
    ...
    this.logger.info(`SimpleMindNodeManager: ${resetSuccess ? "Reset successful!" : "Reset failed!"}`)
