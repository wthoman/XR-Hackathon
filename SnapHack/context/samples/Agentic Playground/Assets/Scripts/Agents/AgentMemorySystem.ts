/**
 * Specs Inc. 2026
 * Agent Memory System component for the Agentic Playground Spectacles lens.
 */
import {
  ChatMessage,
  DiagramState,
...
export class AgentMemorySystem {
  ...
  constructor(logger?: Logger) {
    ...
    try {
      if (typeof global !== "undefined" && global.persistentStorageSystem) {
        ...
        this.logger?.warn("PersistentStorage not available, using in-memory storage")
  ...
  public addMessage(message: Message): void {
    ...
    if (this.messages.length > 500) {
      ...
      this.logger?.info("Message history truncated to prevent memory overflow")
    }

    this.saveToStorage()
    this.logger?.info(`Added message: ${message.role} - ${message.content.substring(0, 50)}...`)
  }
  ...
  public clearMessages(): void {
    this.messages = []
    this.saveToStorage()
    this.logger?.info("Messages cleared")
  }
  ...
  public setSummaryContext(summaryData: SummaryData): void {
    ...
    this.saveToStorage()
    this.logger?.info(`Summary context updated: ${summaryData.summaries.length} sections`)
  }
  ...
  public clearSummaryContext(): void {
    ...
    this.saveToStorage()
    this.logger?.info("Summary context cleared")
  }
  ...
  public addChatMessage(message: ChatMessage): void {
    ...
    if (this.chatHistory.length > 100) {
      ...
      this.logger?.info("Chat history truncated to prevent memory overflow")
    }

    this.saveToStorage()
    this.logger?.info(`Added chat message: ${message.type} - ${message.content.substring(0, 50)}...`)
  }
  ...
  public setDiagramState(diagramState: DiagramState): void {
    ...
    this.saveToStorage()
    this.logger?.info(`Diagram state updated: ${diagramState.nodes.length} nodes`)
  }
  ...
  public clearDiagramState(): void {
    ...
    this.saveToStorage()
    this.logger?.info("Diagram state cleared")
  }
  ...
  public startNewSession(type: "lecture" | "study" | "review", context?: EducationalContext): void {
    ...
    this.saveToStorage()
    this.logger?.info(`New ${type} session started: ${this.currentSession.id}`)
  }
  ...
  public endCurrentSession(): void {
    if (this.currentSession) {
      ...
      this.saveToStorage()
      this.logger?.info(`Session ended: ${this.currentSession.id}`)
    }
  }
  ...
  public saveToStorage(): void {
    try {
      ...
      this.logger?.info(`Storage saved successfully (${processingTime}ms)`)
    } catch (error) {
  ...
  public loadFromStorage(): void {
    try {
      ...
      const processingTime = Date.now() - startTime
      this.logger?.info(`Storage loaded successfully (${processingTime}ms)`)
    } catch (error) {
  ...
  public clearStorage(): void {
    try {
      if (!this.persistentStorage) {
        this.logger?.info("No persistent storage available to clear")
        this.initializeMemory()
        return
      }
  ...
  private saveComponent(key: string, data: any): void {
    ...
    if (data !== null && data !== undefined) {
      ...
      if (size > this.MAX_STORAGE_SIZE / 4) {
        ...
        this.logger?.info(`Component ${key} exceeds size limit, truncating...`)
      ...
      try {
        ...
        this.logger?.error(`Failed to save component ${key}: ${error}`)
      }
    }
  }
