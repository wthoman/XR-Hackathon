/**
 * Specs Inc. 2026
 * Model Node component for the Agentic Playground Spectacles lens.
 */
...
export class ModelNode extends BaseScriptComponent {
  @ui.label('<span style="color: #60A5FA;">ModelNode</span>')
  @ui.separator
  ...
  onAwake(): void {
    this.logger = new Logger("ModelNode", this.enableLogging || this.enableLoggingLifecycle, true)
  ...
  public async generateContent(prompt: string): Promise<void> {
    ...
    if (!this.enable3DGeneration) {
      this.logger.info(`3D model generation disabled for node ${this.nodeId}`)
      return
    }
    ...
    if (this.useGenerationQueue) {
      this.logger.info(`📥 Using scheduled generation for node ${this.nodeId}`)

      // We need our own ModelGenBridge
      if (!this.modelGenerator) {
        this.logger.info(`ModelGenBridge not assigned for node ${this.nodeId}`)
        return
      }

      if (!this.modelPlaceholder) {
        this.logger.info(`Model placeholder not set for node ${this.nodeId}`)
        return
      }
      ...
      const generateWithThisNode = async () => {
        this.logger.info(`Executing scheduled generation for node ${this.nodeId}`)
        ...
        this.modelGenerator.setExternalCallbacks(
          () => {
            ...
            if (this.loadingSpinner) {
              ...
              this.logger.info(`Loading spinner disabled - model received for node ${this.nodeId}`)
            }
          },
          (error: string) => {
        ...
        if (this.modelGenerator.getSceneObject()) {
          ...
          this.logger.info(`📍 Positioned ModelGenBridge at ${placeholderPosition.toString()} for node ${this.nodeId}`)
        }

        // Generate with this node's bridge
        await this.modelGenerator.generateModel(this.modelPrompt, false)

        this.logger.info(`Scheduled generation completed for node ${this.nodeId}`)
      }

      // Schedule the generation
      scheduler.scheduleGeneration(this.nodeId, generateWithThisNode, this.nodeLevel)

      this.logger.info(`Generation scheduled for node ${this.nodeId}`)
      return
    }
    ...
    // Direct generation without scheduling
    {
      ...
      this.logger.info(`Using direct generation for node ${this.nodeId} with prompt: "${this.modelPrompt}"`)
      ...
      try {
        ...
        if (this.modelGenerator.getSceneObject()) {
          ...
          this.logger.info(`ModelGenBridge scene object not found for node ${this.nodeId}`)
        }
        ...
        this.logger.info(`Successfully delegated 3D model generation for node ${this.nodeId}`)
      } catch (error) {
        this.logger.error(`Error in 3D model generation for node ${this.nodeId}: ${error}`)
  ...
  public setModelScale(scale: number): void {
    if (this.modelGenerator) {
      ...
      this.logger.info(`Updated model scale for node ${this.nodeId} to ${scale}`)
    }
  }
  ...
  public clearModels(): void {
    if (this.modelGenerator) {
      ...
      this.logger.info(`Cleared models for node ${this.nodeId}`)
    }
  }
  ...
  public setModel(modelObject: SceneObject): void {
    if (this.modelGenerator && modelObject) {
      ...
      this.logger.info(`Set pre-existing 3D model for node ${this.nodeId}`)
    }
  }
  ...
  public updateModelPosition(): void {
    if (this.modelGenerator && this.modelPlaceholder) {
      ...
      this.logger.info(`Updated model position for node ${this.nodeId}`)
    }
  }
