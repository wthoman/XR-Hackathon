/**
 * Specs Inc. 2026
 * Text Node component for the Agentic Playground Spectacles lens.
 */
...
export class TextNode extends BaseScriptComponent {
  @ui.label('<span style="color: #60A5FA;">TextNode</span>')
  @ui.separator
  ...
  onAwake(): void {
    this.logger = new Logger("TextNode", this.enableLogging || this.enableLoggingLifecycle, true)
  ...
  public async generateContent(prompt: string): Promise<void> {
    ...
    // But this method exists for consistency with other node types
          ...
          this.logger.info(`Content generation called for node ${this.nodeId} with prompt: "${prompt}"`)
  ...
  public setAsStartingNode(isStarting: boolean): void {
    ...
    this.logger.info(`Set node ${this.nodeId} as ${nodeType} node`)
  }
