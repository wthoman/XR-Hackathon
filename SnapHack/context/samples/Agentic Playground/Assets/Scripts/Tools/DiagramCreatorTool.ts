/**
 * Specs Inc. 2026
 * Diagram Creator Tool component for the Agentic Playground Spectacles lens.
 */
...
export class DiagramCreatorTool {
  ...
  constructor(languageInterface: AgentLanguageInterface, diagramStorage?: DiagramStorage) {
    ...
    this.logger?.info("DiagramCreatorTool: Diagram creation tool initialized")
  }
  ...
  public setDiagramStorage(diagramStorage: DiagramStorage): void {
    ...
    this.logger?.info("DiagramCreatorTool: Connected to DiagramStorage")
  }
  ...
  public setSummaryStorage(summaryStorage: SummaryStorage): void {
    ...
    this.logger?.info("DiagramCreatorTool: Connected to SummaryStorage")
  }
  ...
  public setChatStorage(chatStorage: ChatStorage): void {
    ...
    this.logger?.info("DiagramCreatorTool: Connected to ChatStorage")
  }
  ...
  public async execute(args: Record<string, unknown>): Promise<{success: boolean; result?: any; error?: string}> {
    ...
    try {
      this.logger?.info(`DiagramCreatorTool: Processing diagram creation request: "${(query as string).substring(0, 50)}..."`)
      ...
      if (this.diagramStorage) {
        ...
        this.logger?.info("DiagramCreatorTool: No DiagramStorage available - diagram will not be persistently displayed")
      }

      // Generate user response
      const userResponse = this.generateUserResponse(diagramNodes, query as string, maxLength as number)

      this.logger?.info(`DiagramCreatorTool: Generated diagram with ${diagramNodes.length} nodes`)
      ...
      this.logger?.info(`DiagramCreatorTool: ERROR - Diagram creation failed: ${error}`)
      return {
        success: false,
  ...
  private async extractTopicsFromAllSources(
    ...
    if (this.summaryStorage) {
      try {
        ...
        this.logger?.info(`DiagramCreatorTool: 📋 Found ${summaries.length} summaries in storage`)
        ...
        if (currentText && currentText.length > 0) {
          this.logger?.info(`DiagramCreatorTool: 📄 Processing current text (${currentText.length} chars)`)
        }
      } catch (error) {
        this.logger?.info(`DiagramCreatorTool: Error accessing summary storage: ${error}`)
      }
    }
    ...
    if (this.chatStorage) {
      try {
        ...
        if (currentSession && currentSession.messages) {
          this.logger?.info(`DiagramCreatorTool: Found ${currentSession.messages.length} messages in chat history`)
        ...
        this.logger?.info(`DiagramCreatorTool: Error accessing chat storage: ${error}`)
      }
    }

    // 3. If no topics found, use AI to generate relevant topics
    if (allTopics.length === 0) {
      this.logger?.info("DiagramCreatorTool: No topics found in storage, generating from query")
    ...
    this.logger?.info(`DiagramCreatorTool: Extracted ${uniqueTopics.length} unique topics`)
    return uniqueTopics
  }
  ...
  private async generateTopicsFromQuery(
    ...
    try {
      ...
      this.logger?.info(`DiagramCreatorTool: Error generating topics from query: ${error}`)
      return []
    }
  }
  ...
  private async generateDiagramNodes(
    ...
    this.logger?.info(`DiagramCreatorTool: Generated ${nodes.length} hierarchical diagram nodes`)
    return nodes
  }
...
Create a JSON structure:
{
  ...
  "mainBranches": [
    {
      ...
      try {
        ...
        this.logger?.info(`DiagramCreatorTool: Failed to parse AI structure, using fallback`)
      }

      // Fallback structure
      return this.createFallbackStructure(topics, maxNodes)
    } catch (error) {
      this.logger?.info(`DiagramCreatorTool: Error generating hierarchical structure: ${error}`)
      return this.createFallbackStructure(topics, maxNodes)
    }
  }
  ...
  private async storeDiagramInStorage(nodes: DiagramNode[], diagramType: string, query: string): Promise<void> {
    ...
    try {
      ...
      this.logger?.info(`DiagramCreatorTool: Failed to store diagram in storage: ${error}`)
    }
  }
