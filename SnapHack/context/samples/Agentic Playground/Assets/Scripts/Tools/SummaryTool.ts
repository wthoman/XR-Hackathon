/**
 * Specs Inc. 2026
 * Summary Tool component for the Agentic Playground Spectacles lens.
 */
...
export class SummaryTool {
  ...
  constructor(languageInterface: AgentLanguageInterface) {
    ...
    this.logger?.info("SummaryTool: 📋 Summary-focused conversation tool initialized")
  }
  ...
  public async execute(args: Record<string, unknown>): Promise<{success: boolean; result?: any; error?: string}> {
    ...
    try {
      this.logger?.info(`SummaryTool: 📋 Processing summary-focused query: "${(query as string).substring(0, 50)}..."`)
      ...
      this.logger?.info(`SummaryTool: Generated summary-focused response`)
      ...
      this.logger?.info(`SummaryTool: ERROR - Summary processing failed: ${error}`)
      return {
        success: false,
  ...
  private async generateSummaryResponse(systemPrompt: string, messages: Message[], maxLength: number): Promise<string> {
    ...
    this.logger?.info("SummaryTool: Requesting response with voice output enabled (textOnly: false)")
