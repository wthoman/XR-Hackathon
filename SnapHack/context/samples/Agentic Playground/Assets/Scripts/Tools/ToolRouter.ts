/**
 * Specs Inc. 2026
 * Tool Router component for the Agentic Playground Spectacles lens.
 */
...
export class ToolRouter {
  ...
  constructor(languageInterface: AgentLanguageInterface, diagramStorage?: DiagramStorage, logger?: Logger) {
    ...
    this.logger?.info(`ToolRouter: 🧠 AI-powered intelligent tool router initialized with ${this.toolIndex.size} indexed tools`)
    this.logger?.info("ToolRouter: 📚 Tools indexed: " + Array.from(this.toolIndex.keys()).join(", "))
  }
  ...
  public setSummaryStorage(summaryStorage: SummaryStorage): void {
    if (this.diagramCreatorTool) {
      ...
      this.logger?.info("ToolRouter: Connected SummaryStorage to DiagramCreatorTool")
    }
  }
  ...
  public setChatStorage(chatStorage: ChatStorage): void {
    if (this.diagramCreatorTool) {
      ...
      this.logger?.info("ToolRouter: Connected ChatStorage to DiagramCreatorTool")
    }
  }
  ...
  private indexTool(key: string, metadata: ToolMetadata): void {
    ...
    this.logger?.info(`ToolRouter: 📖 Indexed tool "${key}" with ${metadata.capabilities.length} capabilities`)
  }
  ...
  public async routeQuery(args: Record<string, unknown>): Promise<{success: boolean; result?: any; error?: string}> {
    ...
    try {
      ...
      if (!selectedTool || !this.toolIndex.has(selectedTool)) {
        this.logger?.info(`ToolRouter: AI selected unknown tool "${selectedTool}", falling back to general_conversation`)
      ...
      this.logger?.info(`ToolRouter: 💡 Reasoning: ${toolMetadata.description}`)

      return await toolMetadata.instance.execute(args)
    } catch (error) {
      this.logger?.info(`ToolRouter: AI routing failed: ${error}`)
...
Respond with ONLY the tool name (e.g., "summary_tool", "diagram_tool", "spatial_tool", "general_conversation").`
    ...
    try {
      ...
      if (!selectedTool) {
        this.logger?.info(`ToolRouter: AI response "${toolName}" didn't match any indexed tool, using general_conversation`)
        return "general_conversation"
      }

      return selectedTool
    } catch (error) {
      this.logger?.info(`ToolRouter: AI routing decision failed: ${error}`)
      return "general_conversation"
    }
  }
