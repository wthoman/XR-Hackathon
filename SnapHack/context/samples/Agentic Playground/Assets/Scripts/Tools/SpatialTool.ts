/**
 * Specs Inc. 2026
 * Spatial Tool component for the Agentic Playground Spectacles lens.
 */
...
export class SpatialTool {
  ...
  constructor(languageInterface: AgentLanguageInterface) {
    ...
    this.logger?.info("SpatialTool: Spatial awareness tool initialized with camera/Gemini support")
  }
  ...
  public async execute(args: Record<string, unknown>): Promise<{success: boolean; result?: any; error?: string}> {
    ...
    try {
      this.logger?.info(`SpatialTool: Processing spatial query: "${(query as string).substring(0, 50)}..."`)
      ...
      this.logger?.info(`SpatialTool: Generated spatially-aware response`)
      ...
      this.logger?.info(`SpatialTool: ERROR - Spatial processing failed: ${error}`)
      return {
        success: false,
  ...
  private async captureAndAnalyzeEnvironment(): Promise<string> {
    if (this.isCapturingImage) {
      this.logger?.info("SpatialTool: Already capturing image, using cached context")
    ...
    try {
      this.logger?.info("SpatialTool: Capturing actual camera frame for visual analysis")
      ...
      this.logger?.info("SpatialTool: Started camera recording for frame capture")
      ...
      this.logger?.info("SpatialTool: Stopped camera recording")

      if (capturedFrame) {
        this.logger?.info("SpatialTool: Successfully captured camera frame for visual analysis")
        return capturedFrame // Return the base64 encoded image
      } else {
        throw new Error("No frame captured")
      }
    } catch (error) {
      this.logger?.info(`SpatialTool: Camera capture failed: ${error}`)
  ...
  private async generateSpatialResponse(
    ...
    try {
      this.logger?.info("SpatialTool: Using AgentLanguageInterface Live API for spatial analysis with audio output")
      ...
      if (imageContext && imageContext.length > 100 && !imageContext.includes("Unable to capture")) {
        this.logger?.info("SpatialTool: Camera frame captured - sending visual input to AI")
        enhancedUserContent +=
        ...
        this.logger?.info("SpatialTool: No camera frame available - using text-only mode")
        enhancedUserContent +=
      ...
      this.logger?.info(`SpatialTool: Sending spatial query to AgentLanguageInterface Live API`)
      ...
      this.logger?.info("SpatialTool: Requesting response with voice output enabled (textOnly: false)")
      ...
      this.logger?.info(`SpatialTool: Received spatial analysis with audio: "${response.content.substring(0, 100)}..."`)

      // Apply character limits
      return TextLimiter.limitText(response.content, maxLength)
    } catch (error) {
      this.logger?.info(`SpatialTool: AgentLanguageInterface Live API failed: ${error}`)
