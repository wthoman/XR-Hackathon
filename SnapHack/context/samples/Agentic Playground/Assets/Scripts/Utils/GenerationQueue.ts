/**
 * Specs Inc. 2026
 * Defines Generation Queue, Generation Request for the Agentic Playground lens.
 */
 ...
 *    - enableLogging: See detailed queue operations in console
 *
 * 5. Benefits:
...
export class GenerationQueue {
  ...
  private constructor() {
    this.logger?.info("GenerationQueue: Initialized generation queue system")
  }
  ...
  public setImageGenerator(generator: (prompt: string) => Promise<any>): void {
    ...
    this.logger?.info("GenerationQueue: Image generator function registered")
  }
  ...
  public setModelGenerator(generator: (prompt: string, position?: any) => Promise<any>): void {
    ...
    this.logger?.info("GenerationQueue: Model generator function registered")
  }
  ...
  public queueImageGeneration(request: GenerationRequest): string {
    ...
    this.logger?.info(`GenerationQueue: 📥 Queued image request "${requestId}" - Queue size: ${this.imageQueue.length}`)
  ...
  public queueModelGeneration(request: GenerationRequest): string {
    ...
    this.logger?.info(`GenerationQueue: 📥 Queued model request "${requestId}" - Queue size: ${this.modelQueue.length}`)
  ...
  private async processImageQueue(): Promise<void> {
    ...
    if (!this.imageGenerator) {
      this.logger?.info("GenerationQueue: No image generator registered")
      return
    }
    ...
    while (this.imageQueue.length > 0 && this.activeImageRequests < this.maxConcurrentImage) {
      ...
      this.logger?.info(`GenerationQueue: Processing image request "${request.id}" - Remaining: ${this.imageQueue.length}`)
  ...
  private async processImageRequest(request: GenerationRequest): Promise<void> {
    try {
      ...
      this.logger?.info(`GenerationQueue: Completed image request "${request.id}"`)
    } catch (error) {
      this.logger?.info(`GenerationQueue: Failed image request "${request.id}": ${error}`)
  ...
  private async processModelQueue(): Promise<void> {
    ...
    if (!this.modelGenerator) {
      this.logger?.info("GenerationQueue: No model generator registered")
      return
    }
    ...
    if (this.modelQueue.length > 0 || this.activeModelRequests > 0) {
      ...
      this.logger?.info(`GenerationQueue: Model queue empty and no active requests`)
    }
  }
  ...
  private async processModelRequest(request: GenerationRequest): Promise<void> {
    try {
      ...
      this.logger?.info(`GenerationQueue: Completed model request "${request.id}"`)
    } catch (error) {
      this.logger?.info(`GenerationQueue: Failed model request "${request.id}": ${error}`)
  ...
  public clearQueues(): void {
    ...
    this.logger?.info("GenerationQueue: All queues cleared")
  }
