/**
 * Specs Inc. 2026
 * Diagram Extensions component for the Agentic Playground Spectacles lens.
 */
...
export class DiagramExtensions {
  ...
  public static setLogger(logger: Logger): void {
    DiagramExtensions.logger = logger
  }
  ...
  public static addTextNode(
    ...
    if (!diagram || !nodeId || !content) {
      DiagramExtensions.logger?.info("DiagramExtensions: Invalid parameters for addTextNode")
      return false
    }
    ...
    try {
      ...
      if (success) {
        DiagramExtensions.logger?.info(`DiagramExtensions: Added text node "${nodeId}" with content: "${content.substring(0, 30)}..."`)
      } else {
        DiagramExtensions.logger?.info(`DiagramExtensions: Failed to add text node "${nodeId}"`)
      }

      return success
    } catch (error) {
      DiagramExtensions.logger?.info(`DiagramExtensions: Error adding text node: ${error}`)
      return false
    }
  }
  ...
  public static addImageNode(
    ...
    if (!diagram || !nodeId || !content) {
      DiagramExtensions.logger?.info("DiagramExtensions: Invalid parameters for addImageNode")
      return false
    }
    ...
    try {
      ...
      if (success) {
        DiagramExtensions.logger?.info(`DiagramExtensions: Added image node "${nodeId}" with content: "${content.substring(0, 30)}..."`)
      } else {
        DiagramExtensions.logger?.info(`DiagramExtensions: Failed to add image node "${nodeId}"`)
      }

      return success
    } catch (error) {
      DiagramExtensions.logger?.info(`DiagramExtensions: Error adding image node: ${error}`)
      return false
    }
  }
  ...
  public static addModelNode(
    ...
    if (!diagram || !nodeId || !content) {
      DiagramExtensions.logger?.info("DiagramExtensions: Invalid parameters for addModelNode")
      return false
    }
    ...
    try {
      ...
      if (success) {
        DiagramExtensions.logger?.info(`DiagramExtensions: Added model node "${nodeId}" with content: "${content.substring(0, 30)}..."`)
      } else {
        DiagramExtensions.logger?.info(`DiagramExtensions: Failed to add model node "${nodeId}"`)
      }

      return success
    } catch (error) {
      DiagramExtensions.logger?.info(`DiagramExtensions: Error adding model node: ${error}`)
      return false
    }
  }
  ...
  public static addConnection(diagram: DiagramComponent, fromNodeId: string, toNodeId: string): boolean {
    if (!diagram || !fromNodeId || !toNodeId) {
      DiagramExtensions.logger?.info("DiagramExtensions: Invalid parameters for addConnection")
      return false
    }
    ...
    try {
      ...
      if (success) {
        DiagramExtensions.logger?.info(`DiagramExtensions: Added connection from "${fromNodeId}" to "${toNodeId}"`)
      } else {
        DiagramExtensions.logger?.info(`DiagramExtensions: Failed to add connection: ${fromNodeId} -> ${toNodeId}`)
      }

      return success
    } catch (error) {
      DiagramExtensions.logger?.info(`DiagramExtensions: Error adding connection: ${error}`)
      return false
    }
  }
  ...
  public static getNodeCount(diagram: DiagramComponent): number {
    ...
    try {
      ...
      DiagramExtensions.logger?.info(`DiagramExtensions: Error getting node count: ${error}`)
      return 0
    }
  }
  ...
  public static nodeExists(diagram: DiagramComponent, nodeId: string): boolean {
    ...
    try {
      ...
      DiagramExtensions.logger?.info(`DiagramExtensions: Error checking node existence: ${error}`)
      return false
    }
  }
  ...
  public static isReady(diagram: DiagramComponent): boolean {
    ...
    try {
      ...
      DiagramExtensions.logger?.info(`DiagramExtensions: Error checking readiness: ${error}`)
      return false
    }
  }
  ...
  public static clearDiagram(diagram: DiagramComponent): boolean {
    ...
    try {
      // Use the public method from AgenticDiagram
      diagram.resetSystem()
      DiagramExtensions.logger?.info("DiagramExtensions: Cleared all nodes from diagram")
      return true
    } catch (error) {
      DiagramExtensions.logger?.info(`DiagramExtensions: Error clearing diagram: ${error}`)
      return false
    }
  }
  ...
  public static calculateNextPosition(diagram: DiagramComponent, level: number = 1): vec3 {
    ...
    try {
      ...
      DiagramExtensions.logger?.info(`DiagramExtensions: Error calculating position: ${error}`)
      return vec3.zero()
    }
  }
  ...
  public static getDiagramStatus(diagram: DiagramComponent): {
    ...
    try {
      ...
      DiagramExtensions.logger?.info(`DiagramExtensions: Error getting diagram status: ${error}`)
      return {
        isInitialized: false,
        nodeCount: 0,
        textNodes: 0,
