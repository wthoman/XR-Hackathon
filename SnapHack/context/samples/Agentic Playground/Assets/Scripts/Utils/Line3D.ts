/**
 * Specs Inc. 2026
 * Line3 D component for the Agentic Playground Spectacles lens.
 */
...
export class Line3D extends BaseScriptComponent {
  ...
  onAwake(): void {
    this.logger = new Logger("Line3D", this.enableLogging || this.enableLoggingLifecycle, true)
    ...
    if (this.meshVisual) {
      if (this.material) {
        ...
        try {
          ...
          this.logger.info("Could not update material color - " + e)
        }
      } else {
        this.logger.warn("Warning - No material assigned. Please assign a material in the Inspector.")
      }
    }
  }
  ...
  private generateMesh(): void {
    if (!this.pathPoints || this.pathPoints.length < 2) {
      this.logger.info("Need at least 2 path points to generate mesh")
      return
    }
    ...
    this.logger.info("Using " + (this._useWorldPosition ? "WORLD" : "LOCAL") + " coordinate space")
    if (this._zOffset !== 0) {
      this.logger.info("Applied Z-axis offset of " + this._zOffset + " to all path points")
    }
    ...
    if (this.meshBuilder.isValid()) {
      ...
      this.updateMaterial()
      this.logger.info("Mesh generated successfully with " + this.meshBuilder.getVerticesCount() + " vertices")
    } else {
      this.logger.info("Generated mesh data is invalid")
    }
  }
  ...
  private getPathPositions(): vec3[] {
    if (this._debugPositions) {
      ...
      this.logger.info("Using " + (this._useWorldPosition ? "WORLD" : "LOCAL") + " positions")
      this.logger.info("Auto-correct offset: " + this._autoCorrectOffset)
      this.logger.info("Use relative to component: " + this._useRelativeToComponent)
      this.logger.info("Manual Z-offset = " + this._zOffset)
    }
  ...
  private calculateAutomaticOffset(firstPosition: vec3): void {
    ...
    if (this._useWorldPosition && this._useRelativeToComponent) {
      ...
      if (this._debugPositions) {
        this.logger.info("Auto-offset disabled when using world positions with component transformation")
      }
      return
    }
  ...
  public forceRefresh(): void {
    this.logger.info("Force refreshing mesh...")
    this.generateMesh()
  }
  ...
  public setTestPositions(): void {
    ...
    this.logger.info("Setting spline test positions for smooth curve visualization")
    this.logger.info("Interpolation steps = " + this._interpolationSteps + ", Smoothness = " + this._smoothness)
  ...
  public resetOffsetDetection(): void {
    ...
    this.detectedOffset = vec3.zero()
    this.logger.info("Offset detection reset. Will recalculate on next mesh generation.")
    this.updateMesh()
  }

  public testPositionModes(): void {
    this.logger.info("=== Testing Different Position Modes ===")

    // Test world positions
    this.logger.info("Testing WORLD positions...")
    this._useWorldPosition = true
    this._useRelativeToComponent = false
    this.resetOffsetDetection()

    // Test local positions
    this.logger.info("Testing LOCAL positions...")
    this._useWorldPosition = false
    this._useRelativeToComponent = false
    this.resetOffsetDetection()

    // Test world positions with component relative
    this.logger.info("Testing WORLD positions with component transformation...")
    this._useWorldPosition = true
  ...
  public setManualOffset(offset: vec3): void {
    ...
    this.offsetCalculated = true
    this.logger.info("Manual offset set to (" + offset.x + ", " + offset.y + ", " + offset.z + ")")
    this.updateMesh()
  }
