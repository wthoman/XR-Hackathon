/**
 * Specs Inc. 2026
 * Text Limiter component for the Agentic Playground Spectacles lens.
 */
...
export class TextLimiter {
  ...
  public static logLimitViolation(content: string, limit: number, type: string): void {
    ...
    if (info.isOverLimit) {
      TextLimiter.logger?.info(`TextLimiter: Character limit exceeded for ${type}: ${info.length}/${limit} chars`)
      TextLimiter.logger?.info(`TextLimiter: 📝 Content preview: "${content.substring(0, 50)}..."`)
    }
  }
