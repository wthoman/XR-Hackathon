/**
 * Specs Inc. 2026
 * AISummarizer component for the Agentic Playground Spectacles lens.
 */
...
export class AISummarizer extends BaseScriptComponent {
  @ui.label('<span style="color: #60A5FA;">AISummarizer</span>')
  @ui.separator
  ...
  onAwake(): void {
    this.logger = new Logger("AISummarizer", this.enableLogging || this.enableLoggingLifecycle, true)
  ...
  public async generateSummary(text: string, title?: string): Promise<SummaryResult> {
    ...
    this.onProcessingStarted.invoke(this.currentRequestId)
          ...
          this.logger.info(`Starting summarization of ${text.length} characters`)
    ...
    try {
      ...
      this.logger.info(`Summary completed in ${processingTime}ms, generated ${result.sections.length} sections`)
  ...
  private async performSummarization(text: string, title?: string): Promise<SummaryResult> {
    ...
    const request: OpenAITypes.ChatCompletions.Request = {
      ...
      response_format: {type: "json_object"}
          ...
          this.logger.info(`Sending request to OpenAI (${this.model})`)
...
Each section should provide maximum educational value within the character limits.`
  ...
  private parseOpenAIResponse(responseContent: string, originalLength: number): SummaryResult {
    try {
      this.logger.info(`🔍 Parsing response: ${responseContent.substring(0, 200)}...`)

      const parsedResponse = JSON.parse(responseContent)

      if (!parsedResponse.sections || !Array.isArray(parsedResponse.sections)) {
        this.logger.info(`Invalid response format. Parsed object: ${JSON.stringify(parsedResponse)}`)
      ...
      return {
        ...
        this.logger.info(`Raw response was: ${responseContent}`)
