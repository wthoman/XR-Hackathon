/**
 * Specs Inc. 2026
 * YOLODetection Processor component for the SnapML Starter Spectacles lens.
 */
...
export class YOLODetectionProcessor {
  ...
  public initialize(outputs: OutputPlaceholder[], inputs: InputPlaceholder[]): void {
    ...
    if (this.debugLogging) {
      this.logger.debug(`Initialized YOLO processor with ${outputs.length} outputs`)
      this.logger.debug(`Input shape: ${this.inputShape.x}x${this.inputShape.y}x${this.inputShape.z}`)

      for (let i = 0; i < outputs.length; i++) {
        const shape = outputs[i].shape
        this.logger.debug(`Output ${i}: Shape ${shape.x}x${shape.y}x${shape.z}`)
      }
    }
  }
  ...
  public parseYolo7Outputs(outputs: OutputPlaceholder[]): Detection[] {
    ...
    if (this.debugLogging) {
      if (this.boxes.length === 0) {
        ...
        this.logger.debug(`Found ${this.boxes.length} detections from ${totalBoxes} potential boxes`)
      }
    }
