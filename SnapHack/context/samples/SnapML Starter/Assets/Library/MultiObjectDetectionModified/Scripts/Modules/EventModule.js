/**
 * Specs Inc. 2026
 * @module EventModule Module providing helper classes for using javascript based events. @author Snap
 * Inc. @version 1.0.1 ==== Example ==== @example Import module var eventModule =
 * require("./EventModule"); Event Wrapper var myEvent = new eventModule.EventWrapper(); Add callback
 * to the event myEvent.add(function(arg){ print("event triggered with param: " + arg); }); Trigger the
 * event myEvent.trigger("hello!"); Keyed Event Wrapper var keyedEvent = new
 * eventModule.KeyedEventWrapper(); Add callback for "eventA" keyedEvent.add("eventA", function(){
 * print("eventA was triggered!"); }); Add callback for "eventB" keyedEvent.add("eventB", function(){
 * print("eventB was triggered!"); }); Trigger the events keyedEvent.trigger("eventA");
 * keyedEvent.trigger("eventB"); ==== API ==== @api class EventWrapper() Simple implementation of an
 * event class. Add callbacks to be notified when the event is triggered. EventWrapper.add(function
 * callback): function Add a callback function to this event. The callback function will be executed
 * when this event is triggered. EventWrapper.remove(function callback) Remove a callback function from
 * this event. EventWrapper.trigger(arguments...) Trigger the event so that all callbacks are executed.
 * All arguments given will be passed to the callbacks. class KeyedEventWrapper() Simple implementation
 * of a key-based event class. KeyedEventWrapper.getWrapper(string key, boolean createIfMissing=false)
 * Return an EventWrapper for the given `key`. The EventWrapper holds all callbacks added with the same
 * `key`, and is triggered when `trigger` is called with the same `key`. KeyedEventWrapper.add(string
 * key, function callback): function Add a callback function tied to the given `key`. The callback
 * function will be executed when this KeyedEventWrapper is triggered using the same `key`.
 * KeyedEventWrapper.remove(string key, function callback) Remove a callback function tied to the given
 * `key`. KeyedEventWrapper.addAny(function callback): function Add a callback function that will be
 * executed any time a trigger occurs. The first argument for the callback function is the key, the
 * rest of the arguments are what get passed to the trigger. KeyedEventWrapper.removeAny(function
 * callback): function Remove a callback function that was added using `addAny()`.
 * KeyedEventWrapper.trigger(string key, arguments...) Trigger all callback functions that were added
 * using the same `key`. All arguments after `key` will be passed to the callback functions.
 */
if (!isNull(script)) {
