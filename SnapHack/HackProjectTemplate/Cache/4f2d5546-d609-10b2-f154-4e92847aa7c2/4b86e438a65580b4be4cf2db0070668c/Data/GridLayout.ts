...
export class GridLayout extends BaseScriptComponent {
  ...
  public layoutBy: LayoutDirection

  public transform: Transform = this.getTransform()
   ...
   */
  public getCellSceneObject = (x: number, y: number): SceneObject => {
    let i
    if (this.layoutBy === LayoutDirection.Column) {
      i = x * this.rows
      i += y % this.rows
   ...
   */
  public calculateChildPosition(gridColumn: number, gridRow: number): vec3 {
