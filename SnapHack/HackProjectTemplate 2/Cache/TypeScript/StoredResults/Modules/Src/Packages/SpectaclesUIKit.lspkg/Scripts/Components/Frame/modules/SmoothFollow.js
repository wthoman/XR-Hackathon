...
const translationTime = 0.45;
const rotationTime = 0.55;
/**
...
class SmoothFollow {
    constructor(options) {
        ...
        this.transform = this.frame.transform;
        this.fieldOfView = 26;
        this.tangentHalfFOV = Math.tan((this.fieldOfView / 2) * mathUtils_1.DegToRad);
    ...
    clampPosition() {
        ...
        this.target.z = (0, mathUtils_1.clamp)(this.target.z, minDistance, maxDistance);
        this.target.z = Math.max(this.target.z, (1.1 * this.visibleWidth) / 2 / this.tangentHalfFOV); // handle very wide panels
        this.target.y = (0, mathUtils_1.clamp)(this.target.y, minElevation, maxElevation);
    ...
    onUpdate() {
        if (!this.dragging) {
            ...
            this.target.y = (0, mathUtils_1.clamp)(pos.y, minElevation, maxElevation);
            [pos.x, this.velocity.x] = (0, mathUtils_1.smoothDamp)(pos.x, this.target.x, this.velocity.x, translationTime, getDeltaTime());
            [pos.y, this.velocity.y] = (0, mathUtils_1.smoothDamp)(pos.y, this.target.y, this.velocity.y, translationTime, getDeltaTime());
            [pos.z, this.velocity.z] = (0, mathUtils_1.smoothDamp)(pos.z, this.target.z, this.velocity.z, translationTime, getDeltaTime());
            this.worldPos = this.bodyToWorld(this.cylindricalToCartesian(pos));
            [this.heading, this.omega] = (0, mathUtils_1.smoothDampAngle)(this.heading, this.cameraHeading, this.omega, rotationTime, getDeltaTime());
            // force billboard
